    ...
    <div className="min-h-screen bg-gray-900 relative">
      ...
      <div className="relative max-w-4xl mx-auto p-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
          <h1 className="text-4xl font-bold text-white text-center mb-2">Teacher Dashboard</h1>
        ...
        {profileData && (
          ...
          >
            ...
            <div className="bg-white bg-opacity-15 p-3 rounded-lg">
              ...
              <p className="text-base text-white mb-2"><span className="font-semibold text-white text-opacity-80">Subject: </span>{profileData.subject || 'Not available'}</p>
              <p className="text-base text-white"><span className="font-semibold text-white text-opacity-80">Experience: </span>{profileData.experience_years || '0'} years</p>
            </div>
          </motion.div>
        )}
        <motion.button
          className="w-full bg-indigo-600 text-white py-4 rounded-full text-lg font-bold hover:bg-indigo-700"
          onClick={() => navigate('/teacher/booking', { state: { userData } })}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
